...
void nrf_init(unsigned short spi, char MODE){
...
{
	int i = 0;
	char status = nrf_cmd_rd(spi,NRF_CONFIG);
	if(((status & 1)==1) | (R_GP(PB,0)==0) | ((status & 0x2)==0))
	{
		nrf_ptx_init(spi);
	}
	spi_cs(spi,LOW);
...
// nrf_send
char nrf_send(unsigned short spi, char payload[])
{
	nrf_rf_tx(spi,payload);
	DelayMs(10);
	char status = nrf_cmd_rd(1,NRF_STATUS);
...
void nrf_rx(unsigned short spi, char payload[])
{
	nrf_rf_rx(spi,payload);
}

char nrf_msg_check(unsigned short spi)
{
	char status = nrf_cmd_rd(1,NRF_STATUS);
